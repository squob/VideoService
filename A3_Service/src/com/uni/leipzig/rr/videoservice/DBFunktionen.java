...
public class DBFunktionen extends SQLiteOpenHelper {
	...
	private final static String TableName = "MP4DB51";
...
//				+ kommentar);
	...
	public JSONObject dbAbfrageJSON(JSONObject jso) {
		...
		if (details.isEmpty()) {
			...
			sDetails += " Details LIKE '%" + details + "%'";

			// Log.v(TAG, "DEtails:"+sDetails);
		}

		Log.v("Blödersinne", sWhereClausel + sGroesse + sTitel + sDetails);
		try {
			Cursor c = db.rawQuery("SELECT DISTINCT Ort FROM " + TableName
					+ sWhereClausel + sGroesse + sTitel + sDetails + ";", null);
			// Check if our result was valid.
			Log.v(TAG, "" + c.getColumnCount());
			Log.v(TAG, "" +"SELECT DISTINCT Titel Ort " + TableName
					+ sWhereClausel + sGroesse + sTitel + sDetails + ";" );
		 if (c.getCount()!= 0) {
				int Column1 = c.getColumnIndex("Ort");
				c.moveToFirst();
				Log.v("Tolle Anzahl", c.getCount() +"");
				do {
					titel = c.getString(Column1);
		...
		if(list == null){
			...
			try {
				for (int i = 0; i < list.size(); i++) {
					jsa.put(list.get(i));
//					 Log.v("EINGEFÜGT",list.get(i) + " eingefügt");
				}
				jso.put("Songs", jsa);
	...
	public void erstelleDB() {
		...
		db.execSQL("CREATE TABLE IF NOT EXISTS "
				+ TableName
				+ " (Ort VARCHAR,Titel VARCHAR,Dimension VARCHAR , Hash VARCHAR, Laenge VARCHAR, Groesse LONG, Details VARCHAR);");
